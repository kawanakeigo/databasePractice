...
-- 「Arubaの人口は103000人です」
SELECT CONCAT(name, 'の人口は', population, '人です') AS message FROM countries;
...
SELECT * FROM countries ORDER BY life_expectancy DESC NULLS LAST, indep_year DESC NULLS LAST;
...
SELECT c.name, cl.language, ci.name AS city_name FROM countries c JOIN countrylanguages cl ON c.code = cl.country_code JOIN cities ci ON c.code = ci.country_code;

-- 問27
-- 全ての有名人を出力してください。左側外部結合を使用して国名なし（country_codeがNULL）も表示してください。
SELECT ce.*, co.name AS country_name FROM celebrities ce LEFT JOIN countries co ON ce.country_code = co.code;

-- 問28
-- 全ての有名人の名前,国名、第一言語を出力してください。
SELECT ce.name, co.name AS country_name, cl.language FROM celebrities ce JOIN countries co ON ce.country_code = co.code JOIN countrylanguages cl ON co.code = cl.country_code AND cl.is_official = 'T';

-- 問29
-- 全ての有名人の名前と国名をに出力してください。 ただしテーブル結合せずサブクエリを使用してください。
SELECT name, (SELECT name FROM countries c WHERE c.code = ce.country_code) FROM celebrities ce;

-- 問30
-- 最年長が50歳以上かつ最年少が30歳以下の国を表示させてください。
SELECT c.name FROM countries c JOIN celebrities ce ON c.code = ce.country_code GROUP BY c.name HAVING MAX(ce.age) >= 50 AND MIN(ce.age) <= 30;

-- 問31
-- 1991年生まれと、1981年生まれの有名人が何人いるか調べてください。ただし、日付関数は使用せず、UNION句を使用してください。
SELECT COUNT(*) FROM celebrities WHERE birth LIKE '1991-%' UNION SELECT COUNT(*) FROM celebrities WHERE birth LIKE '1981-%';

-- 問32
-- 有名人の出身国の平均年齢を高い方から順に表示してください。ただし、FROM句はcountriesテーブルとしてください。
SELECT co.name, AVG(ce.age) FROM countries co JOIN celebrities ce ON co.code = ce.country_code GROUP BY co.name ORDER BY AVG(ce.age) DESC;